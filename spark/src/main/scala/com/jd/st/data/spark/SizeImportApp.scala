...
object SizeImportApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    dataImport(sc, tablePrefix, pyramid, path)

    sc.stop()
  }

  private def xianData(sc: SparkContext, path: String): RDD[GeodeticFeature] = {
  ...
  private def dataImport(sc: SparkContext,
    ...
    val featureRDD: RDD[GeodeticFeature] = xianData(sc, path)
    ...
    // 存储统计量数据
    val countRDD = featureRDD.mapPartitions(iter => {
      iter.map(feature => {
        val pair = bcPyramid.value.locateFeature(feature)
        val binTile = pair.getKey
        (binTile.binEpochSeconds, (binTile.z2,1))
      })
    }).groupBy(_._1)
      .mapPartitions(iter => {
        iter.map(tuple => {
